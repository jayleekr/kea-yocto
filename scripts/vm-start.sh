...
if [ "$ARCH" != "x86_64" ]; then
    log_error "이 스크립트는 x86_64 VM 전용입니다."
    echo "현재 시스템: $ARCH"
    echo "다른 스크립트를 사용하세요:"
    echo "  - ARM64 Mac: ./scripts/simple-start.sh"
...
# Docker 이미지 다운로드 (x86_64 명시)
DOCKER_IMAGE="jabang3/yocto-lecture:5.0-lts"
log_step "x86_64 Docker 이미지 다운로드 중..."

# 이미지가 없거나 ARM64 버전이면 강제로 x86_64 다운로드
if ! docker image inspect $DOCKER_IMAGE >/dev/null 2>&1; then
    log_info "이미지를 다운로드합니다..."
    docker pull --platform linux/amd64 $DOCKER_IMAGE
elif docker image inspect $DOCKER_IMAGE | grep -q "arm64"; then
    log_warning "ARM64 이미지가 감지되어 x86_64 버전을 다운로드합니다..."
    docker rmi $DOCKER_IMAGE 2>/dev/null || true
    docker pull --platform linux/amd64 $DOCKER_IMAGE
fi

# 이미지 아키텍처 확인
log_step "이미지 아키텍처 확인 중..."
IMAGE_ARCH=$(docker image inspect $DOCKER_IMAGE | grep -o '"Architecture":"[^"]*"' | cut -d'"' -f4)
log_info "이미지 아키텍처: $IMAGE_ARCH"

if [ "$IMAGE_ARCH" != "amd64" ]; then
    log_error "잘못된 아키텍처의 이미지가 다운로드되었습니다!"
    echo "예상: amd64, 실제: $IMAGE_ARCH"
    log_step "올바른 이미지 강제 다운로드 중..."
    docker rmi $DOCKER_IMAGE 2>/dev/null || true
    docker pull --platform linux/amd64 $DOCKER_IMAGE
fi
